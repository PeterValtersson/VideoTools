...
namespace VideoTools.Services.Controllers
{
    ...
    public class DownloaderController(ILogger<Test> logger, IProcessServiceRequestQueue _processQueue, IOptions<AppSettings> options) : ControllerBase
    {
        // POST
        [HttpPost("add")]
        public async Task<IActionResult> Add([FromBody] JSONRequestData request, CancellationToken cancellationToken)
        {
            logger.LogInformation("Donwloader Add endpoint called. Name: {}, Url: {}",
